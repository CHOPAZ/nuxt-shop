<script setup lang="ts">
  ...
  const { data: categoriesData } = await useFetch<ICategoriesGET>(API_URL + '/categories')

  //Получение карточек продуктов
  const { data: productData } = await useFetch<IProductsGET>(API_URL + '/products', {
    key: 'get-products',
    query,
  })
  ...
  <div class="catalog">
    ...
    <div class="catalog__content">
      ...
      <div class="catalog__cards">
        <CatalogCard v-for="product in productData?.products" :key="product.id" v-bind="product" />
      </div>
    </div>
  </div>
</template>
